-- 0030_order_items_created_index.sql — Index cho MẪU SỐ của tỷ lệ hủy (REPORT-10).
--
-- LÝ DO: `report_cancel_summary` (0029) tính `ordered_qty` bằng subquery lọc `order_items` theo
-- (tenant_id, created_at). Index có sẵn chỉ phủ vế TỬ SỐ — `idx_order_items_cancelled` là partial
-- `where status = 'cancelled'` (0028) — còn index còn lại là (tenant_id, order_id). Không có
...
--
-- Không sửa 0028/0029 (đã áp lên dev) — thêm index ở migration mới.
