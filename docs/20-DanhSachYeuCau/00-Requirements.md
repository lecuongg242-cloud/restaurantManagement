...
| OPS-07 | Chốt chặn lệch schema trong CI | CI dựng shadow DB từ `supabase/migrations/` rồi `supabase db diff` với production (schema `public`,`storage`); khác nhau → **CI đỏ** kèm nội dung diff. Đối chứng âm: tạo tay một object trên DB test mà không viết migration → CI phải đỏ; xoá đi → CI xanh lại. Thiếu secrets thì bước này **skip**, không đỏ giả | P8 | ◐ job `schema-drift` đã thêm, **chế độ chỉ báo cáo**; chưa bật chặn merge và chưa chạy đối chứng âm vì `db diff` cần Docker (máy dev không có). Các bước phải chạy: `40-KiemTra/OPS-07-DoiChungAm.md` |
